...
- **Caching**: editorial pages stay fully SSG; per-user state only via client islands hitting `/api/me/state`. On publish/update, invalidate through `src/lib/revalidate.ts` — never hand-roll tag/path lists; it must cover both locales' paths. Draft filtering AND the en-subset rule (en-visible ⇔ zh + en published) happen in `src/db/queries/public.ts`.
- **i18n**: zh at the root, English under `/en` as a second root-layout tree (ADR 0010) — new public routes must be added to both trees. Shared components take a `locale` prop; UI strings for shared chrome live in `src/i18n/dictionaries/` (server-only; pass labels to client islands as props). No zh prose may render on `/en`.
