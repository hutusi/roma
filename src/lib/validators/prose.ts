...
import { isAllowedLinkHref } from "@/components/tiptap/link-policy";
  ...
  if (Array.isArray(node.marks)) {
    for (const mark of node.marks) {
      if (
        ...
        !isAllowedLinkHref((mark as { attrs?: Record<string, unknown> }).attrs?.href)
      ) {
        return "富文本链接仅支持 http(s) 或 mailto 地址";
      }
    }
  }
