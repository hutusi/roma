...
export const seedFilms: SeedFilm[] = [
  // ── Federico Fellini ──────────────────────────────────────────────
  {
    ...
    wikidataId: "Q12018",
    editorialNote:
      "费里尼开拍时手里有合同、有档期，没有题材，于是把这个处境本身写成了剧本。圭多是个导演，布景搭好了，剧本始终没有，记忆、欲望与负疚就住进了这块空地。1963 年真正新鲜、如今已成常规的是它的剪接：梦境、回忆与当下在同一个镜头里换位，不作交代，也不给过渡的提示。摄影上，光被派去承担叙事。圭多的白衬衫、修女的黑袍、浴场蒸汽里的一片过曝，读出来都是情绪的刻度。马斯楚安尼这个角色大半靠听戏完成，比看上去难，也是全片得以立住的地方。结尾那场马戏团式的圆圈舞，让圭多与自己拍不出电影这件事达成和解：比他出发时想要的那种要小，也是唯一还能拿到的。此后六十年关于创作者自我怀疑的电影，在诚实这一项上没有再往前走多少。",
    editorialNoteEn:
      "Fellini began with a contract, a start date and no subject, and wrote that condition into the screenplay. Guido is a director with a set already built and a script that never arrives; memory, desire and guilt move into the vacancy. What was new in 1963 and is ordinary now is the cutting: dream, recollection and the present tense change places inside a single shot, with no announcement and no transitional cue. Light does the narrating. Guido's white shirt, the black habits of the nuns, a blown-out patch of steam at the spa register as emotional readings rather than as rendered rooms. Marcello Mastroianni plays the part largely by listening, which is harder than it looks and is what holds the picture together. The circus-ring dance at the end reconciles Guido with his inability to film his life \u2014 a smaller peace than the one he set out for, and the only one on offer. Sixty years of films about blocked artists have not moved much past its honesty on that subject.",
    essay: doc([
      h2("困境如何成为形式"),
      p(
  ...
  // ── Yasujirō Ozu ──────────────────────────────────────────────────
  {
    ...
    wikidataId: "Q26060",
    editorialNote:
      "平山夫妇从尾道到东京看望子女，住了几天，被辗转安排到各处，最后由战死儿子的遗孀纪子接待。小津把摄影机放在跪坐的高度，很少移动，也很少给近景。争执始终没有发生，怠慢也从未被点破，一切都在礼貌的对话里完成，这是它扎人的原因。母亲回乡后不久去世，父亲独自留在空屋，说了一句今天又要热了。影片对亲情的判断就藏在这种平淡里：感情会随距离与时间变薄，而当事人往往要到无法补救时才察觉。原节子演的纪子是唯一的例外，她的善意也并不轻松，影片没有替她把这件事解决掉。",
    editorialNoteEn:
  ...
  },
  {
    ...
    wikidataId: "Q1056853",
    editorialNote:
      "苏丽珍与周慕云是邻居，各自的配偶正与对方的配偶来往。两人为了弄清这种事是怎么开始的，用排演的方式互相扮演那两个人，排着排着，分寸就不好把握了。王家卫把承重的情节全部拿掉：偷情没有拍，表白没有说出口，两位配偶的脸自始至终不在画面里。留下的是楼梯上的错身、走廊、慢镜头里的烟，以及梅林茂那段反复回来的圆舞曲。张曼玉换了二十多件旗袍，颜色替她说她不能说的话。结尾周慕云在吴哥窟对着石缝低语，观众一个字也听不见。影片讲克制，但并不赞美克制：这两个人没做的事，花掉了他们的后半生。",
    editorialNoteEn:
      "Su Li-zhen and Chow Mo-wan are neighbours whose spouses are conducting an affair with each other. To work out how such a thing begins, they rehearse it, each playing the other's husband or wife, and the rehearsal stops being safe. Wong removed every load-bearing plot point: the adultery is not shown, the feeling is never declared, and the two unfaithful spouses never appear on camera. What is left is the near-misses on a staircase, the corridors, the smoke in slow motion, and Shigeru Umebayashi's waltz coming back and coming back. Maggie Cheung's twenty-odd cheongsams do the talking her character is not allowed to do. At Angkor Wat, Chow whispers into a hollow in the stone and the audience hears none of it. The film is about restraint and is not admiring of it. What these two decline to do costs them the rest of their lives.",
    essay: doc([
      h2("克制作为一种纪律"),
      p(
  ...
  },
  {
    ...
    wikidataId: "Q2057159",
    editorialNote:
      "白天是母亲，夜里站街。吴永刚写这个剧本时二十七岁，第一次执导，处理方式在当年很少见：不控诉，不怜悯，机位与人物平齐，连勒索她的流氓也只当环境的一部分来拍，没有被安排成供人痛骂的反派。阮玲玉的表演几乎不依赖字幕卡——她把攒下的钞票一张张塞进墙砖缝里，这个动作已经把尊严说清楚了。中国默片的成就在这部片子上见了顶。次年阮玲玉自杀，二十四岁。今天重看，仍然站得住的是影片对待主人公的态度：她被当作一个人来拍，而 1934 年前后，几乎没有别的电影愿意这样对待处在她位置上的女性。",
    editorialNoteEn:
      "By day a mother, by night a streetwalker. Wu Yonggang wrote the script at twenty-seven. He was directing for the first time. His handling of the material was uncommon for 1934: no indictment, no pity, the camera set level with its subject. Even the gangster who extorts her is filmed as part of the environment, not staged as someone to hiss at. Ruan Lingyu works almost without intertitles. She pushes her saved banknotes one at a time into a gap in the brickwork, and the gesture has already explained what dignity means. Chinese silent film reached its ceiling here. Ruan killed herself the following year, at twenty-four. What holds up on a return visit is the film's posture toward its heroine: she is filmed as a person, at a moment when very little cinema anywhere was prepared to extend that to a woman in her position.",
    essay: doc([
      h2("平视的高度"),
      p(
