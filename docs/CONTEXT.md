...
- `src/db/seed-data/` + the content CLIs — the editorial corpus is checked-in TypeScript, written by `db:seed:content` (idempotent, `onConflictDoNothing`, never clobbers admin edits). It tags the films it creates itself, and refuses to start if one references a tag the admin-owned vocabulary lacks. Siblings for the cases it cannot handle: `apply-tags.ts` repairs tags on an admin-owned vocabulary and acts **only on the `--films=`/`--create-tags=` you name — it never infers intent from DB state**, since neither a missing tag nor an untagged film distinguishes a gap from an editorial decision (ADR 0014 amendment), `resync-content.ts` overwrites content fields of named slugs, `backfill-en.ts` fills NULL/draft English fields, `link-cast.ts` links existing cast rows to people. All run **outside Next**, so none can call `revalidate.ts` — redeploy after writing. See DEPLOY.md's content runbook.
