...
- `src/lib/revalidate.ts` — the single map from entity → cache invalidation. Publish actions call it; readers see changes without redeploys (and it pings IndexNow in prod).
