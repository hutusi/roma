...
| 影片介绍 / 人物介绍 | The descriptive body; plain text, counted in Unicode code points (CJK-correct). Neutral and encyclopedic — every sentence should be the kind of claim a source could support — and the field that gates publishing. On people this is `bio`, which doubles as the card blurb and meta description, so it stays short and 创作历程 carries the length (ADR 0017). |
| 编辑札记 | The one field carrying an editorial view, in an editor's own voice. Optional, and capped rather than floored: a film publishes on its introduction alone. The old 200-code-point *minimum* is what made one sharp sentence unpublishable and manufactured prose the only way to satisfy the form; a ceiling cannot be met by writing more. Length belongs in 长文 (ADR 0017). |
...
| 片单 (curated list) | THE core product. Theme + intro essay + per-film 入选理由 + deliberate ordering ("顺序即立场"). All three prose fields are neutral: the selection and the order carry the editorial position, so the words do not have to (ADR 0017). URL namespace `/list/` is editorial-only. |
...
| 英文版 (English edition) | Per-entity `*En` fields + `statusEn`. Listings/sitemap/RSS/JSON-LD on `/en` expose only the en-published subset (en-visible ⇔ published in zh AND en), but detail URLs never 404 for en-pending entities — they render a noindex translation-pending stub linking to the zh edition (ADR 0012). English introductions gate on 120–350 words; the English note is optional and capped. Both editions are authored, never translated (ADR 0010), which is checkable: matching sentence counts across the two are a sign something went wrong. Users carry an optional `locale` ("zh"/"en", NULL for pre-column accounts) captured at sign-up and editable in /account; the reset email is single-language when the locale is known, bilingual otherwise. |
