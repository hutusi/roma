...
- (2026-07-18 review follow-up) Role names split like person names, NOT like prose: `character` is the Latin/original form (`/en` shows it alone), `characterZh` the zh form (`/zh` prefers it, falling back to `character`). A `characterEn` prose-style column was rejected — it would have hidden every existing Latin role from `/en` until hand-duplicated. Migration 0008 moved CJK/kana values into `characterZh`; the TMDB import fetches en credits for `character` and keeps the zh-CN role only when it actually differs.
