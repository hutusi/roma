...
- (2026-07-12 follow-up) Language switching became first-class chrome: a header toggle derives the counterpart URL by locale-prefix swap (`counterpartPath`; query strings dropped — `/films`' `country` param is locale-mapped), and a dismissible browser-language hint banner links mismatched readers to the other edition — client-side only, never a redirect, so the static serving path stays untouched. `users.locale` (captured at sign-up, editable in /account) retires ADR 0010's bilingual reset-email stopgap for accounts with a known locale.
